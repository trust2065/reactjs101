...
4. 更好地處理觸摸事件（Touch Handling）
5. 使用 Flexbox 進行排版和布局
6. 良好的可擴展性（Extensibility），容易整合 Web 生態系標準（XMLHttpRequest、 navigator.geolocation 等）或是原生的元件或函式庫  
7. Facebook 已使用 React Native 於自家 Production App 且將持續維護
8. 持續蓬勃發展的技術社群
9. 讓 Web 開發者可以使用熟悉的技術切入 Native App 開發
