...
1. 使用 `browserify` 或 `webpack` 等 `CommonJS` bundler 整合 `babel` 預處理
2. 於瀏覽器端做解析

在這邊我們先使用第二種方式，先讓大家熟悉 JSX 語法使用，等到後面章節在教大家使用 bundler 的方式去做解析（可以試著把下面的原始碼貼到 [JSbin](http://jsbin.com/) 的 HTML 看結果）：
...
### 6. 擴展屬性
在 ES6 中使用 `...` 是迭代物件的意思，可以把所有物件對應的值迭代出來設定屬性，注意後面設定的屬性會蓋掉前面相同屬性：
...
## 總結
以上就是 JSX 簡明入門教學，希望透過以上介紹，讓讀者了解在 React 中為何要使用 JSX。 
